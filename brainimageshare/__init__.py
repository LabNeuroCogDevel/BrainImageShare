...
def ni_to_img(ni_mat, i=None, j=None, k=None, ratio=1):
    ...
    """
    if i is None:
        i = int(ni_mat.shape[0] / 2)
    if j is None:
        j = int(ni_mat.shape[1] / 2)
    if k is None:
        k = int(ni_mat.shape[2] / 2)
...
def mk_image_stack(nii_jpg, lncd_template):
    ...
    total_size = (
        max(lncd_template.size[0], nii_jpg.size[0]),
...
def brainimageshare():
    ...
    if len(sys.argv) > 1:
        ...
        t1_file = askopenfilename(
            defaultextension=".nii.gz", title="select subject mprage.nii.gz"
        )
        root.update()
